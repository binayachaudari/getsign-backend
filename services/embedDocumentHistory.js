const FileDetails = require('../models/FileDetails');
const FileHistory = require('../models/FileHistory');
const PdfWriter = require('../utils/PdfWriter');

const STATUS_MAPPER = {
  sent: 'Sent to',
  resent: 'Resent to',
  viewed: 'Viewed by',
  signed_by_sender: 'Signed By',
  signed_by_receiver: 'Signed By',
};

const FILE_HISTORY_STATUS = {
  sent: 'sent',
  resent: 'resent',
  signed_by_sender: 'signed_by_sender',
  signed_by_receiver: 'signed_by_receiver',
  viewed: 'viewed',
};

const AUDIT_TRIAL_CONFIG = {
  fontSize: {
    documentDetailHeading: 10,
    documentDetailSubheading: 8,
  },
  documentPadding: {
    x: 50,
    y: 20,
    linePaddingY: 2,
    newLinePaddingY: 6,
  },
  imgDimensions: {
    height: 32,
    width: 32,
  },
};

const getAllHistories = fileHistories => {
  let sentHistories = [];
  let resentHistories = [];
  let viewedHistoires = [];
  let signedHisotries = [];

  for (const history of fileHistories) {
    const status = history.status;

    switch (status) {
      case FILE_HISTORY_STATUS.sent:
        sentHistories.push(history);
        continue;
      case FILE_HISTORY_STATUS.resent:
        resentHistories.push(history);
        continue;
      case FILE_HISTORY_STATUS.signed_by_receiver:
      case FILE_HISTORY_STATUS.signed_by_sender:
        signedHisotries.push(history);
        continue;
      case FILE_HISTORY_STATUS.viewed:
        viewedHistoires.push(history);
        continue;
      default:
        continue;
    }
  }

  return {
    sentHistories,
    resentHistories,
    viewedHistoires,
    signedHisotries,
  };
};

const addHistory = (
  pdfDoc,
  page,
  currentXCoordinate,
  currentYCoordinate,
  text,
  fontSize
) => {
  if (
    currentYCoordinate - AUDIT_TRIAL_CONFIG.documentPadding.y <= 0 ||
    currentYCoordinate - fontSize <= 0
  ) {
    page = pdfDoc.addPage();
    currentYCoordinate =
      page.getSize().height - AUDIT_TRIAL_CONFIG.documentPadding.y;
  }

  page.drawText(text, {
    x: currentXCoordinate,
    y: currentYCoordinate - fontSize,
    size: fontSize,
  });

  currentYCoordinate -=
    fontSize + AUDIT_TRIAL_CONFIG.documentPadding.linePaddingY;

  return currentYCoordinate;
};

module.exports = {
  newEmbedHistory: async (pdfDoc, fileId, itemId) => {
    const docHistory = await FileHistory.find({ fileId, itemId }).sort({
      created_at: 'asc',
    });
    const docDetails = await FileDetails.findById(fileId);

    const { sentHistories, signedHisotries, viewedHistoires, resentHistories } =
      getAllHistories(docHistory);

    let page = pdfDoc.addPage();
    const { height } = page.getSize();

    const documnetDetailText = [
      {
        text: 'Document Details',
        fontSize: AUDIT_TRIAL_CONFIG.fontSize.documentDetailHeading,
      },
      {
        text: `FileID: ${fileId}`,
        fontSize: AUDIT_TRIAL_CONFIG.fontSize.documentDetailSubheading,
      },
      {
        text: `Filename: ${docDetails.file_name}`,
        fontSize: AUDIT_TRIAL_CONFIG.fontSize.documentDetailSubheading,
      },
      {
        text: `Creation Date: ${new Date(
          docDetails.created_at
        ).toLocaleDateString('en-US')}`,
        fontSize: AUDIT_TRIAL_CONFIG.fontSize.documentDetailSubheading,
      },
    ];

    let currentYCoordinate = height;
    // Write Document Details
    for (const [index, docDetailText] of documnetDetailText.entries()) {
      const currentLinePaddingY =
        index === 0
          ? AUDIT_TRIAL_CONFIG.documentPadding.y
          : AUDIT_TRIAL_CONFIG.documentPadding.linePaddingY;

      const options = {
        x: AUDIT_TRIAL_CONFIG.documentPadding.x,
        y: currentYCoordinate - currentLinePaddingY - docDetailText.fontSize,
        size: docDetailText.fontSize,
      };
      page.drawText(docDetailText.text, options);
      currentYCoordinate -= currentLinePaddingY + docDetailText.fontSize;
    }

    currentYCoordinate -= AUDIT_TRIAL_CONFIG.documentPadding.newLinePaddingY;
    // Write Audit Trail Heading
    page.drawText('Audit Trail', {
      x: 50,
      y: currentYCoordinate - AUDIT_TRIAL_CONFIG.fontSize.documentDetailHeading,
      size: AUDIT_TRIAL_CONFIG.fontSize.documentDetailHeading,
    });

    currentYCoordinate -=
      AUDIT_TRIAL_CONFIG.fontSize.documentDetailHeading +
      2 * AUDIT_TRIAL_CONFIG.documentPadding.newLinePaddingY;

    const addNewpageAndrResetY = () => {
      page = pdfDoc.addPage();
      currentYCoordinate = height - AUDIT_TRIAL_CONFIG.documentPadding.y;
    };

    for (const [index, history] of sentHistories.entries()) {
      if (index === 0) {
        if (currentYCoordinate - AUDIT_TRIAL_CONFIG.imgDimensions.height <= 0) {
          addNewpageAndrResetY();
        }

        const sent = await pdfDoc.embedPng(
          'data:image/png;base64,'
        );

        page.drawImage(sent, {
          x: AUDIT_TRIAL_CONFIG.documentPadding.x,
          y: currentYCoordinate - AUDIT_TRIAL_CONFIG.imgDimensions.height,
          height: AUDIT_TRIAL_CONFIG.imgDimensions.height,
          width: AUDIT_TRIAL_CONFIG.imgDimensions.width,
        });

        page.drawText('Sent', {
          x:
            AUDIT_TRIAL_CONFIG.documentPadding.x +
            AUDIT_TRIAL_CONFIG.imgDimensions.width +
            23 * 0.75,
          y:
            currentYCoordinate -
            AUDIT_TRIAL_CONFIG.fontSize.documentDetailHeading,
          size: AUDIT_TRIAL_CONFIG.fontSize.documentDetailHeading,
        });
        currentYCoordinate -=
          AUDIT_TRIAL_CONFIG.fontSize.documentDetailHeading +
          AUDIT_TRIAL_CONFIG.documentPadding.newLinePaddingY;
      }

      let currentXCoordinate =
        AUDIT_TRIAL_CONFIG.documentPadding.x +
        AUDIT_TRIAL_CONFIG.imgDimensions.width +
        23 * 0.75;

      const text = `${
        STATUS_MAPPER[history.status] + ' ' + (history.sentToEmail || '')
      }`;
      const fontSize = AUDIT_TRIAL_CONFIG.fontSize.documentDetailSubheading;
      currentYCoordinate = addHistory(
        pdfDoc,
        page,
        currentXCoordinate,
        currentYCoordinate,
        text,
        fontSize
      );
      const dateTime = `${new Date(
        history.created_at
      ).toLocaleDateString()} ${new Date(
        history.created_at
      ).toLocaleTimeString()}`
        .replace(' ', ' ')
        .toString();

      currentYCoordinate = addHistory(
        pdfDoc,
        page,
        currentXCoordinate,
        currentYCoordinate,
        `${dateTime} UTC`,
        AUDIT_TRIAL_CONFIG.fontSize.documentDetailSubheading
      );

      if (index === sentHistories.length - 1) {
        currentYCoordinate -=
          2 * AUDIT_TRIAL_CONFIG.documentPadding.newLinePaddingY -
          AUDIT_TRIAL_CONFIG.documentPadding.linePaddingY;
      }
    }

    for (const [index, history] of resentHistories.entries()) {
      if (index === 0) {
        if (currentYCoordinate - AUDIT_TRIAL_CONFIG.imgDimensions.height <= 0) {
          addNewpageAndrResetY();
        }

        const sent = await pdfDoc.embedPng(
          'data:image/png;base64,'
        );

        page.drawImage(sent, {
          x: AUDIT_TRIAL_CONFIG.documentPadding.x,
          y: currentYCoordinate - AUDIT_TRIAL_CONFIG.imgDimensions.height,
          height: AUDIT_TRIAL_CONFIG.imgDimensions.height,
          width: AUDIT_TRIAL_CONFIG.imgDimensions.width,
        });

        page.drawText('Resent', {
          x:
            AUDIT_TRIAL_CONFIG.documentPadding.x +
            AUDIT_TRIAL_CONFIG.imgDimensions.width +
            23 * 0.75,
          y:
            currentYCoordinate -
            AUDIT_TRIAL_CONFIG.fontSize.documentDetailHeading,
          size: AUDIT_TRIAL_CONFIG.fontSize.documentDetailHeading,
        });
        currentYCoordinate -=
          AUDIT_TRIAL_CONFIG.fontSize.documentDetailHeading +
          AUDIT_TRIAL_CONFIG.documentPadding.newLinePaddingY;
      }

      let currentXCoordinate =
        AUDIT_TRIAL_CONFIG.documentPadding.x +
        AUDIT_TRIAL_CONFIG.imgDimensions.width +
        23 * 0.75;

      const text = `${
        STATUS_MAPPER[history.status] + ' ' + (history.sentToEmail || '')
      }`;
      const fontSize = AUDIT_TRIAL_CONFIG.fontSize.documentDetailSubheading;
      currentYCoordinate = addHistory(
        pdfDoc,
        page,
        currentXCoordinate,
        currentYCoordinate,
        text,
        fontSize
      );
      const dateTime = `${new Date(
        history.created_at
      ).toLocaleDateString()} ${new Date(
        history.created_at
      ).toLocaleTimeString()}`
        .replace(' ', ' ')
        .toString();

      currentYCoordinate = addHistory(
        pdfDoc,
        page,
        currentXCoordinate,
        currentYCoordinate,
        `${dateTime} UTC`,
        AUDIT_TRIAL_CONFIG.fontSize.documentDetailSubheading
      );

      if (index === resentHistories.length - 1) {
        currentYCoordinate -=
          2 * AUDIT_TRIAL_CONFIG.documentPadding.newLinePaddingY -
          AUDIT_TRIAL_CONFIG.documentPadding.linePaddingY;
      }
    }

    for (const [index, history] of viewedHistoires.entries()) {
      if (index === 0) {
        if (currentYCoordinate - AUDIT_TRIAL_CONFIG.imgDimensions.height <= 0) {
          addNewpageAndrResetY();
        }

        const viewed = await pdfDoc.embedPng(
          'data:image/png;base64,'
        );

        page.drawImage(viewed, {
          x: AUDIT_TRIAL_CONFIG.documentPadding.x,
          y: currentYCoordinate - AUDIT_TRIAL_CONFIG.imgDimensions.height,
          height: AUDIT_TRIAL_CONFIG.imgDimensions.height,
          width: AUDIT_TRIAL_CONFIG.imgDimensions.width,
        });

        page.drawText('Viewed', {
          x:
            AUDIT_TRIAL_CONFIG.documentPadding.x +
            AUDIT_TRIAL_CONFIG.imgDimensions.width +
            23 * 0.75,
          y:
            currentYCoordinate -
            AUDIT_TRIAL_CONFIG.fontSize.documentDetailHeading,
          size: AUDIT_TRIAL_CONFIG.fontSize.documentDetailHeading,
        });
        currentYCoordinate -=
          AUDIT_TRIAL_CONFIG.fontSize.documentDetailHeading +
          AUDIT_TRIAL_CONFIG.documentPadding.newLinePaddingY;
      }

      let currentXCoordinate =
        AUDIT_TRIAL_CONFIG.documentPadding.x +
        AUDIT_TRIAL_CONFIG.imgDimensions.width +
        23 * 0.75;

      const text = `${
        STATUS_MAPPER[history.status] + ' ' + (history.sentToEmail || '')
      }`;
      const fontSize = AUDIT_TRIAL_CONFIG.fontSize.documentDetailSubheading;
      currentYCoordinate = addHistory(
        pdfDoc,
        page,
        currentXCoordinate,
        currentYCoordinate,
        text,
        fontSize
      );
      const dateTime = `${new Date(
        history.created_at
      ).toLocaleDateString()} ${new Date(
        history.created_at
      ).toLocaleTimeString()}`
        .replace(' ', ' ')
        .toString();

      currentYCoordinate = addHistory(
        pdfDoc,
        page,
        currentXCoordinate,
        currentYCoordinate,
        `${dateTime} UTC`,
        AUDIT_TRIAL_CONFIG.fontSize.documentDetailSubheading
      );

      currentYCoordinate = addHistory(
        pdfDoc,
        page,
        currentXCoordinate,
        currentYCoordinate,
        `IP ${history.viewedIpAddress}`,
        AUDIT_TRIAL_CONFIG.fontSize.documentDetailSubheading
      );

      if (index === viewedHistoires.length - 1) {
        currentYCoordinate -=
          2 * AUDIT_TRIAL_CONFIG.documentPadding.newLinePaddingY -
          AUDIT_TRIAL_CONFIG.documentPadding.linePaddingY;
      }
    }

    for (const [index, history] of signedHisotries.entries()) {
      if (index === 0) {
        if (currentYCoordinate - AUDIT_TRIAL_CONFIG.imgDimensions.height <= 0) {
          addNewpageAndrResetY();
        }

        const signed = await pdfDoc.embedPng(
          'data:image/png;base64,'
        );

        page.drawImage(signed, {
          x: AUDIT_TRIAL_CONFIG.documentPadding.x,
          y: currentYCoordinate - AUDIT_TRIAL_CONFIG.imgDimensions.height,
          height: AUDIT_TRIAL_CONFIG.imgDimensions.height,
          width: AUDIT_TRIAL_CONFIG.imgDimensions.width,
        });

        page.drawText('Signed', {
          x:
            AUDIT_TRIAL_CONFIG.documentPadding.x +
            AUDIT_TRIAL_CONFIG.imgDimensions.width +
            23 * 0.75,
          y:
            currentYCoordinate -
            AUDIT_TRIAL_CONFIG.fontSize.documentDetailHeading,
          size: AUDIT_TRIAL_CONFIG.fontSize.documentDetailHeading,
        });
        currentYCoordinate -=
          AUDIT_TRIAL_CONFIG.fontSize.documentDetailHeading +
          AUDIT_TRIAL_CONFIG.documentPadding.newLinePaddingY;
      }

      let currentXCoordinate =
        AUDIT_TRIAL_CONFIG.documentPadding.x +
        AUDIT_TRIAL_CONFIG.imgDimensions.width +
        23 * 0.75;

      const text = `${
        STATUS_MAPPER[history.status] + ' ' + (history.sentToEmail || '')
      }`;
      const fontSize = AUDIT_TRIAL_CONFIG.fontSize.documentDetailSubheading;
      currentYCoordinate = addHistory(
        pdfDoc,
        page,
        currentXCoordinate,
        currentYCoordinate,
        text,
        fontSize
      );
      const dateTime = `${new Date(
        history.created_at
      ).toLocaleDateString()} ${new Date(
        history.created_at
      ).toLocaleTimeString()}`
        .replace(' ', ' ')
        .toString();

      currentYCoordinate = addHistory(
        pdfDoc,
        page,
        currentXCoordinate,
        currentYCoordinate,
        `${dateTime} UTC`,
        AUDIT_TRIAL_CONFIG.fontSize.documentDetailSubheading
      );

      currentYCoordinate = addHistory(
        pdfDoc,
        page,
        currentXCoordinate,
        currentYCoordinate,
        `IP ${history.receiverSignedIpAddress}`,
        AUDIT_TRIAL_CONFIG.fontSize.documentDetailSubheading
      );

      if (index === signedHisotries.length - 1) {
        currentYCoordinate -=
          2 * AUDIT_TRIAL_CONFIG.documentPadding.newLinePaddingY -
          AUDIT_TRIAL_CONFIG.documentPadding.linePaddingY;
      }
    }

    return pdfDoc;
  },
  embedHistory: async (pdfDoc, fileId, itemId) => {
    const gap = 32;

    const docHistory = await FileHistory.find({ fileId, itemId }).sort({
      created_at: 'asc',
    });
    const docDetails = await FileDetails.findById(fileId);

    const { sentHistories, signedHisotries, viewedHistoires, resentHistories } =
      getAllHistories(docHistory);

    let page = pdfDoc.addPage();
    const { height } = page.getSize();
    const fontSize = 30;
    let currentIconPos = {
      x: 50,
      y: height - 250,
    };

    // Document Details
    page.drawText('Document Details', {
      x: 50,
      y: height - 20 - fontSize,
      size: 18,
    });

    page.drawText(`FileID: ${fileId}`, {
      x: 50,
      y: height - 50 - fontSize,
      size: 16,
    });

    page.drawText(`Filename: ${docDetails.file_name}`, {
      x: 50,
      y: height - 70 - fontSize,
      size: 16,
    });

    page.drawText(
      `Creation Date: ${new Date(docDetails.created_at).toLocaleDateString(
        'en-US'
      )}`,
      {
        x: 50,
        y: height - 90 - fontSize,
        size: 16,
      }
    );

    // Audit Trail
    page.drawText('Audit Trail', {
      x: 50,
      y: height - 150 - fontSize,
      size: 18,
    });

    const signed = await pdfDoc.embedPng(
      'data:image/png;base64,'
    );

    const sent = await pdfDoc.embedPng(
      'data:image/png;base64,'
    );

    const viewed = await pdfDoc.embedPng(
      'data:image/png;base64,'
    );

    let currentDetailPos = {
      x: currentIconPos.x + signed.width + gap,
      y: height - 220,
    };

    docHistory
      ?.filter(item => item.status !== 'resent')
      .forEach(history => {
        if (currentDetailPos.y <= 0 || currentIconPos.y <= 0) {
          page = pdfDoc.addPage();
          currentIconPos = {
            x: 50,
            y: height - 250,
          };

          currentDetailPos = {
            x: currentIconPos.x + signed.width + gap,
            y: height - 220,
          };
        }

        switch (history.status) {
          case 'signed_by_sender':
          case 'signed_by_receiver':
            page.drawImage(signed, {
              x: currentIconPos.x,
              y: currentIconPos.y,
            });
            break;

          case 'sent':
            page.drawImage(sent, {
              x: currentIconPos.x,
              y: currentIconPos.y,
            });
            break;

          case 'viewed':
            page.drawImage(viewed, {
              x: currentIconPos.x,
              y: currentIconPos.y,
            });
            break;
        }
        // updaing current icon pos
        currentIconPos = {
          x: currentIconPos.x,
          y: currentIconPos.y - 100,
        };

        // Adding details with icons
        page.drawText(
          `${
            STATUS_MAPPER[history.status] + ' ' + (history.sentToEmail || '')
          }`,
          {
            x: currentDetailPos.x,
            y: currentDetailPos.y,
            size: 18,
          }
        );

        const dateTime = `${new Date(
          history.created_at
        ).toLocaleDateString()} ${new Date(
          history.created_at
        ).toLocaleTimeString()}`
          .replace(' ', ' ')
          .toString();

        page.drawText(`${dateTime} UTC`, {
          x: currentDetailPos.x,
          y: currentDetailPos.y - 20,
          size: 14,
        });

        switch (history.status) {
          case 'viewed':
            page.drawText(`IP ${history.viewedIpAddress}`, {
              x: currentDetailPos.x,
              y: currentDetailPos.y - 40,
              size: 14,
            });
            break;

          case 'signed_by_receiver':
            page.drawText(`IP ${history.receiverSignedIpAddress}`, {
              x: currentDetailPos.x,
              y: currentDetailPos.y - 40,
              size: 14,
            });
            break;
        }

        currentDetailPos = {
          x: currentDetailPos.x,
          y: currentDetailPos.y - 100,
        };
      });

    return pdfDoc;
  },
};
